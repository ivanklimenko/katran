...
1. **План «совместимость»** — собрать в один план по итогам спайков 2026-09-24 и расхождений с эталоном:
   ...
   - **Встраивание.** Спайк `spike/federation` (от `spike/react17`, в работе): макет хоста webpack 5 MF с React 17 singleton и своим effector, remote на ките; сборка `@katran/ui` под `chrome88` и проверка `dist` на синтаксис; сборка `dist` у `@katran/effector` (сейчас пакет указывает на `.ts`-исходники); варианты shared для effector; CSS, шрифты, порталы.
