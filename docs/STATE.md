...
- **Ветка `feat/slice1-grid` = план 2 исполнен целиком** (`docs/superpowers/plans/2026-09-23-katran-slice1-grid.md`, 14 задач, последний коммит `1ca7663`), worktree `.worktrees/feat-slice1-grid`, база `7696740`. Состав: `DataGrid` с семейством (типы, `resolveSpans`, `sortRows`, выделение, `ColumnHeader`, `ColumnsMenu`, клавиатура, скелетон), пакет `@katran/effector` (`createFiltersModel`, `createGridModel`, хуки, persist), демо «Реестр», Playwright-замер. Финальное ревью ветки (Opus) → фикс-волна из 6 коммитов → повторное ревью фикс-волны: замечаний нет. Документы (спека, план, CHANGELOG, README, этот файл) приведены к коду отдельным коммитом.
- **Проверки на `1ca7663`:** 160 тестов (tokens 12, ui 125, effector 23), `pnpm check` зелёный; e2e 3/3 — запись 72 px, шапка 49, скелетон 72, плотность 125 % → 89.75.
- **Ждёт:** слияние `feat/slice1-grid` в `main` и push — по решению владельца. На `origin/feat/slice1-grid` сейчас `fe75371`, ветка локально впереди.
- **Открытые решения владельца** (спека 12): эталон высоты записи — ≈ 65 px в спеке против 72 по замеру: снять число со стенда `pi-constructor`, затем сузить коридор e2e или принять 72; Pages на приватном репо (сделать публичным / ручной запуск / оставить красным); буква `StatusDot` на светлых тонах (контраст 3.0–3.2).
...
- **Hover подсвечивает `tr`, а не всю запись** (`tbody`) — у многострочной записи подсвечивается одна строка.
...
1. Слить `feat/slice1-grid` в `main` и запушить (решение владельца). После слияния worktree `.worktrees/feat-slice1-grid` можно удалить; вместе с ним уходит леджер плана 2 (`.superpowers/sdd/2026-09-23-katran-slice1-grid/` — `progress.md` с 13 решениями контроллера, брифы и отчёты задач, находки финального ревью; git-ignored). Всё, что из него нужно дальше, перенесено в спеку и этот файл; если леджер нужен как история — скопировать до удаления.
